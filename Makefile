...
STATIC_TOOL_DIR := $(BUILD_DIR)/static
...
STATIC_TOOL_BINS := $(addprefix $(STATIC_TOOL_DIR)/, $(patsubst %.o,%,$(notdir $(TOOL_OBJS))))
...
# add if hdf5 not in /usr/include
...
ALL_BUILD_DIRS := $(sort $(BUILD_DIR) $(addprefix $(BUILD_DIR)/, $(SRC_DIRS)) \
	...
	$(DISTRIBUTE_SUBDIRS) $(PROTO_BUILD_INCLUDE_DIR) $(STATIC_TOOL_DIR))
...
tools: $(TOOL_BINS) $(TOOL_BIN_LINKS) $(STATIC_TOOL_BINS)
...
$(STATIC_TOOL_BINS): $(STATIC_TOOL_DIR)/% : $(TOOL_BUILD_DIR)/%.o | $(STATIC_NAME)
	@ echo CXX/LD -o $@
	$(Q)$(CXX) $< -o $@ $(STATIC_LINK_COMMAND) $(LDFLAGS)
